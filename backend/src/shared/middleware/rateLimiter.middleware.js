...
function createStore(windowMs) {
  ...
  const interval = setInterval(() => {
    const now = Date.now();
    for (const [key, expiry] of hits) { if (expiry < now) hits.delete(key); }
  }, windowMs);
...
function rateLimiter(options = {}) {
  ...
  return (req, res, next) => {
    ...
    if (!record || record.expiry < now) {
      store.hits.set(key, { count: 1, expiry: now + config.windowMs });
    ...
    if (record.count > config.max) {
      res.setHeader('Retry-After', Math.ceil((record.expiry - now) / 1000));
